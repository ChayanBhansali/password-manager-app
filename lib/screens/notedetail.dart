...
class _NoteDetailState extends State<NoteDetail> {
  ...
  Widget build(BuildContext context) {
    titleController.text = note.platform!;
    descController.text = note.username!;
    return Scaffold(
      appBar: AppBar(
        title: Text("texxt"),
      ),
  ...
  void updateTitle(){
    note.platform = titleController.text;
  }

  // Update the description of Note object
  void updateDescription() {
    note.username = descController.text;
  }
  void _delete() async {
    ...
    int? result = await helper.deleteNote(note.time);
    if (result != 0) {
  ...
  void _save() async {
    ...
    note.time = DateFormat.yMMMd().format(DateTime.now());
    int? result;
    if (note.time != null) {  // Case 1: Update operation
